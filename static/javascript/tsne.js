...
(function(global) {
  ...
  var xtod = function(X, label) {
    ...
    for(var i=0;i<N;i++) {
      for(var j=i+1;j<N;j++) {
        if (label[i] == label[j]){
          dist[i*N+j] = dist[i*N+j] * 0.7;
          dist[j*N+i] = dist[j*N+i] * 0.7;
        }
  ...
  tSNE.prototype = {
    ...
    initDataRaw: function(X, label) {
      var N = X.length;
      var D = X[0].length;
      assert(N > 0, " X is empty? You must have some data!");
      assert(D > 0, " X[0] is empty? Where is the data?");
      var dists = xtod(X, label); // convert X to distances using gaussian kernel
